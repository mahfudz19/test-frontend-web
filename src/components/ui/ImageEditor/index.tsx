...
export function ImageEditorModal({
  ...
  const constrainPosition = (newPosition: { x: number; y: number }, currentScale: number) => {
    if (!naturalDimensions.width || !naturalDimensions.height) return newPosition
  ...
  const generateAvatar = () => {
    if (!image || !canvasRef.current || !imageRef.current || !naturalDimensions.width) return
  ...
  useEffect(() => {
    if (image && imageRef.current && canvasRef.current && naturalDimensions.width > 0) {
      generateAvatar()
    }
    ...
    <div className='flex flex-col items-center w-full max-w-md mx-auto'>
      ...
      {showControls && (
        <div className='w-full space-y-4'>
          ...
          <div className='space-y-2'>
            <div className='flex items-center justify-between'>
              ...
              <div className='flex space-x-2'>
                <IconButton variant='text' onClick={zoomOut} disabled={scale <= getMinScale()}>
                  <IconZoomOut />
                </IconButton>
                <IconButton variant='text' onClick={zoomIn} disabled={scale >= 3}>
                  <IconZoomIn />
                </IconButton>
              </div>
            </div>
          ...
          <p className='text-xs text-gray-500 text-center mt-2'>Geser gambar untuk mengatur posisi</p>

          {/* Action buttons */}
          <div className='flex justify-center gap-2 pt-4'>
            {onCancel && (
              <Button variant='text' startIcon={<IconClose />} onClick={onCancel} color='error'>
                Batal
              </Button>
            )}
            <Button variant='outlined' startIcon={<IconSave />} onClick={handleSave}>
              Simpan Avatar
            </Button>
          </div>
        </div>
    ...
    <div className='flex flex-col items-center w-full max-w-md mx-auto p-4'>
      ...
      <div
        className={`relative overflow-hidden border-2 p-4 ${
          isDragOver ? 'border-primary-main bg-secondary-main/25' : 'border-divider'
      ...
      >
        <div className='flex flex-col items-center justify-center h-full text-gray-500'>
          ...
          <p className='text-sm text-center'>Drag & drop gambar atau klik untuk upload</p>
        </div>
