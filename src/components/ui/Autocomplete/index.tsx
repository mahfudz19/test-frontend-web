...
// 🔹 Props AutoComplete
export interface AutoCompleteProps<T extends WithId>
...
function AutoComplete<T extends WithId>(props: AutoCompleteProps<T>) {
  ...
  const handleRemoveLastSelectedOption = () => {
    setSelectedOptions((prev) => {
      ...
      if (setFieldValue)
        setFieldValue(multiple ? updated : updated[0] || undefined);
  ...
  const scrollToHighlightedOption = (index: number) => {
    setTimeout(() => {
      const popover = document.querySelector(
        ".custom-popover > div.overflow-y-auto"
      ) as HTMLDivElement | null;
      if (!popover) return;

      const optionElements = popover.querySelectorAll(
        '[role="option"]'
    ...
    switch (e.key) {
      ...
      case "Enter":
        ...
        if (highlightedIndex !== -1) {
          ...
          if (
            !highlightedElement ||
            highlightedElement.getAttribute("aria-disabled") === "true"
          )
            return;
  ...
  return (
    <>
      ...
      {multiple && (
        <ul
          className={twMerge(
            "p-1 overflow-auto text-sm max-h-64",
            classNames?.ulRenderFieldArray
          )}
        >
          {(Array.isArray(sortingOrNot) ? sortingOrNot : [])?.map?.(
            (item: T, i: number) => {
              ...
              return (
                <li key={`${i}`} className={classNames?.liRenderFieldArray}>
                  ...
                  {renderFieldArray?.(item, {
                    ...
                    tabIndex: -1,
                    onDelete: () =>
                      !(rest.disabled || rest.readOnly) ? handleSelect() : null,
      ...
      >
        <div
          tabIndex={-1}
          className="outline-none p-2 flex flex-col gap-0.5 max-h-60 overflow-y-auto"
        >
