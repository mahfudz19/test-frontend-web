...
const years = Array.from({ length: length }, (_, i) => today.getFullYear() + 5 - i)
...
const HelperFunction = (
  ...
  const getFirstDayOfMonth = useCallback((month: number, year: number) => new Date(year, month, 1).getDay(), [])

  // Fungsi untuk mengubah Date menjadi ISO string
  const formatToISOString = useCallback((date: Date | null) => (!date ? '' : date.toISOString()), [])
  ...
  const isDateHovered = useCallback(
    (date: Date) => {
      ...
      if (!startDate || !hoveredDate || activeInputRef.current !== 'end') return false
  ...
  const safeOnChange = useCallback(
    (name?: string, value?: string) => onChange?.({ target: { name, value: value ?? '' } }),
    [onChange]
  )
...
const EventHandler = (
  ...
  const handleDateChange = useCallback(
    (type: DateChangeType, value: number | Date | string, dateType: DateInputType = 'start', isCurrentMonth = true) => {
      ...
      if (type === 'selector-month') {
        ...
        setDateState(prev => {
          ...
          const focusedDate = new Date(
            prev.currentYear,
            month,
            Math.min(currentFocusedDate.getDate(), daysInMonth(month, prev.currentYear))
          )
      ...
      if (type === 'selector-year') {
        ...
        setDateState(prev => {
          ...
          const focusedDate = new Date(
            year,
            prev.currentMonth,
            Math.min(currentFocusedDate.getDate(), daysInMonth(prev.currentMonth, year))
          )
          focusedDateRef.current = focusedDate
        ...
        if (selectsRange) {
          if (
            ...
            if (!dateState.startDate || date < dateState.startDate) {
              ...
              const startIso = formatToISOString(date < dateState.startDate! ? date : dateState.startDate)
              const endIso = formatToISOString(date < dateState.startDate! ? dateState.startDate : date)
      ...
      setDateState(prev => {
        ...
        if (type === 'day' && cleanValue.length <= 2) newState[`${prefix}Day`] = cleanValue
        if (type === 'month' && cleanValue.length <= 2) newState[`${prefix}Month`] = cleanValue
        if (type === 'year' && cleanValue.length <= 4) newState[`${prefix}Year`] = cleanValue
          ...
          if (day >= 1 && day <= 31 && month >= 0 && month <= 11 && year >= 1900 && year <= 2100) {
            ...
            if (newDate.getDate() === day && newDate.getMonth() === month && newDate.getFullYear() === year) {
              ...
              if (!isDateDisabled(newDate)) {
                ...
                newState.startHour = newDate.getHours().toString().padStart(2, '0')
                newState.startMinute = newDate.getMinutes().toString().padStart(2, '0')

                // Trigger onChange for single date
                if (!selectsRange) safeOnChange(name, formatToISOString(newDate))
              }
            }
          }
        } else if (
          ...
          if (day >= 1 && day <= 31 && month >= 0 && month <= 11 && year >= 1900 && year <= 2100) {
            ...
            if (newDate.getDate() === day && newDate.getMonth() === month && newDate.getFullYear() === year) {
              ...
              if (!isDateDisabled(newDate)) {
                ...
                newState.endHour = newDate.getHours().toString().padStart(2, '0')
                newState.endMinute = newDate.getMinutes().toString().padStart(2, '0')
  ...
  const handlePrevMonth = useCallback(() => {
    ...
    focusedDateRef.current = focusedDate
    setDateState(prev => ({ ...prev, currentMonth: newMonth, currentYear: newYear, slideDirection: 'right' }))
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dateState.currentMonth, dateState.currentYear, focusedDateRef, daysInMonth, calendarDayRefs, animationTimeoutRef])
  ...
  const handleNextMonth = useCallback(() => {
    ...
    focusedDateRef.current = focusedDate
    setDateState(prev => ({ ...prev, currentMonth: newMonth, currentYear: newYear, slideDirection: 'left' }))
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dateState.currentMonth, dateState.currentYear, focusedDateRef.current, daysInMonth])
  ...
  const handleTimeChange = useCallback(
    (value: string, type: 'hour' | 'minute', dateType: 'start' | 'end' = 'start') => {
        ...
        if (dateType === 'start' && prev.startDate) {
          if (type === 'hour' && value.length === 2) {
            ...
            if (hour >= 0 && hour <= 23) {
              ...
              if (!selectsRange) {
                ...
                safeOnChange(name, `${formatToISOString(newDate)}/${formatToISOString(prev.endDate)}`)
              }
            }
          ...
          if (type === 'hour' && value.length === 2) {
            ...
            if (hour >= 0 && hour <= 23) {
              ...
              if (prev.startDate) {
                safeOnChange(name, `${formatToISOString(prev.startDate)}/${formatToISOString(newDate)}`)
              }
            }
          } else if (type === 'minute' && value.length === 2) {
            const minute = Number.parseInt(value, 10)
  ...
  const toggleMonthSelector = useCallback(() => {
    setDateState(prev => ({
      ...prev,
      focusedElement: prev.focusedElement === 'monthSelector' ? 'monthButton' : 'monthSelector'
    }))
  ...
  const toggleYearSelector = useCallback(() => {
    setDateState(prev => ({
      ...prev,
      focusedElement: prev.focusedElement === 'yearSelector' ? 'yearButton' : 'yearSelector'
    }))
  ...
  const handlerClose = useCallback(() => {
    ...
    setDateState(p => ({
      ...
      currentYear: p.startDate ? p.startDate.getFullYear() : today.getFullYear(),
...
const FocusManagement = (
  ...
  const getFocusableElements = useCallback(() => {
    ...
    // Tombol clear
    if (isClearable && (dateState.startDate || dateState.endDate) && clearButtonRef.current) {
      elements.push(clearButtonRef.current)
    }

    return elements
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [focusedDateRef.current, dateState.startDate, dateState.endDate, showTimeSelect, selectsRange, isClearable])
  ...
  const updateFocusedElement = useCallback((element: HTMLElement) => {
    ...
    if (element === monthButtonRef.current) {
      ...
      const calendarDayButton = Array.from(calendarDayRefs.current.entries()).find(([_, el]) => el === element)
      ...
      if (calendarDayButton) {
        ...
        const hourInput = Array.from(hourInputRefs.current.entries()).find(([_, el]) => el === element)

        if (hourInput) {
          activeInputRef.current = hourInput[0] as 'start' | 'end'
          setDateState(prev => ({ ...prev, focusedElement: 'hourInput' }))
        } else {
          const minuteInput = Array.from(minuteInputRefs.current.entries()).find(([_, el]) => el === element)
    ...
    (e: React.KeyboardEvent) => {
      ...
      if (dateState.focusedElement === 'calendar') {
        if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(key)) {
          ...
          e.preventDefault()
          const firstDayOfMonth = new Date(dateState.currentYear, dateState.currentMonth, 1)
          navigateToDate(firstDayOfMonth)
        } else if (key === 'End') {
          // Navigasi ke akhir bulan
          e.preventDefault()
          const lastDayOfMonth = new Date(dateState.currentYear, dateState.currentMonth + 1, 0)
      ...
      // Handle navigasi di selector bulan
        ...
        if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(key)) {
          ...
          const focusedMonthButton = Array.from(monthButtonsRef.current.entries()).find(
            ([_, el]) => el === document.activeElement
          )

          if (focusedMonthButton) handleDateChange('selector-month', focusedMonthButton[0])
        } else if (key === 'Escape') {
          e.preventDefault()
          e.stopPropagation()
          setDateState(prev => ({ ...prev, focusedElement: 'monthButton' }))
      ...
      // Handle navigasi di selector tahun
        ...
        if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(key)) {
          ...
          const focusedYearButton = Array.from(yearButtonsRef.current.entries()).find(
            ([_, el]) => el === document.activeElement
          )
      ...
      else if (dateState.focusedElement === 'hourInput' || dateState.focusedElement === 'minuteInput') {
        if (key === 'ArrowLeft' && dateState.focusedElement === 'minuteInput') {
          e.preventDefault()
          // Fokus ke input jam
          // setDateState(prev => ({ ...prev, focusedElement: 'hourInput' }))
          hourInputRefs.current.get(activeInputRef.current)?.focus()
        } else if (key === 'ArrowRight' && dateState.focusedElement === 'hourInput') {
          e.preventDefault()
  ...
  const navigateCalendarWithArrowKeys = useCallback(
    (key: string) => {
      ...
      if (newMonth !== dateState.currentMonth || newYear !== dateState.currentYear) {
        // Arahkan slide berdasarkan perbedaan tanggal
        const direction =
          newYear < dateState.currentYear || (newYear === dateState.currentYear && newMonth < dateState.currentMonth)
            ? 'right'
            : 'left'

        // Update bulan saat ini dan fokuskan tanggal baru
        focusedDateRef.current = newDate
        setDateState(prev => ({ ...prev, currentMonth: newMonth, currentYear: newYear, slideDirection: direction }))
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [dateState.currentMonth, dateState.currentYear, focusedDateRef.current, isDateDisabled]
  )
  ...
  const navigateMonthSelector = useCallback(
    (key: string) => {
      const activeElement = document.activeElement
      const currentMonthButton = Array.from(monthButtonsRef.current.entries()).find(([_, el]) => el === activeElement)
  ...
  const navigateYearSelector = useCallback(
    (key: string) => {
      const activeElement = document.activeElement
      const currentYearButton = Array.from(yearButtonsRef.current.entries()).find(([_, el]) => el === activeElement)
...
const RenderFunction = (
  ...
  const renderCalendarDays = useCallback(
    (monthOffset = 0) => {
      ...
      const days: { date: Date; isCurrentMonth: boolean; isPrevMonth: boolean; isNextMonth?: boolean }[] = []

      // Tambahkan hari-hari dari bulan sebelumnya
      for (let i = 0; i < firstDay; i++) {
        const day = daysInPrevMonth - firstDay + i + 1
        days.push({ date: new Date(prevYear, prevMonth, day), isCurrentMonth: false, isPrevMonth: true })
      }

      // Tambahkan hari-hari dari bulan saat ini
      for (let i = 1; i <= totalDays; i++) {
        days.push({ date: new Date(year, month, i), isCurrentMonth: true, isPrevMonth: false, isNextMonth: false })
      }
      ...
      for (let i = 1; i <= remainingCells; i++) {
        ...
        days.push({ date, isCurrentMonth: false, isPrevMonth: false, isNextMonth: true })
      }
          ...
          <div className='grid grid-cols-7 gap-1'>
            {days.map((dayInfo, index) => (
              <CalendarDay
                ...
                onDateClick={(date, month) => handleDateChange('date', date, activeInputRef.current, month)}
  ...
  const renderMonthView = useCallback(
    (monthOffset = 0) => {
      ...
      const registerMonthButtonRef = (el: HTMLButtonElement | null) => (monthButtonRef.current = el)
      const registerYearButtonRef = (el: HTMLButtonElement | null) => (yearButtonRef.current = el)
      ...
      if (dateState.focusedElement === 'monthSelector') {
        return (
          <MonthSelector
            ...
            registerMonthRef={(month, el) => (el ? monthButtonsRef.current.set(month, el) : undefined)}
            monthNames={monthNames}
          />
        )
      }
      ...
      if (dateState.focusedElement === 'yearSelector') {
        return (
          <YearSelector
            ...
            registerYearRef={(year, el) => (el ? yearButtonsRef.current.set(year, el) : undefined)}
          />
        )
      }
      ...
      const renderContent = () => (
        <>
          ...
          {dateState.slideDirection && (
            ...
            >
              <div className={`grid grid-cols-${monthsShown} gap-4 justify-center`}>
                {!(monthsShown > 2 || monthsShown < 1) ? (
                  Array.from({ length: monthsShown }).map((_, index) => (
                    ...
                    >
                      {renderMonthView(dateState.slideDirection === 'left' ? --index : ++index)}
                    </div>
                  ))
                ) : (
                  <></>
          ...
          >
            <div className={`grid grid-cols-${monthsShown} gap-4 justify-center`}>
              {!(monthsShown > 2 || monthsShown < 1) ? (
                Array.from({ length: monthsShown }).map((_, index) => (
                  <div key={`incoming-${index}`} className='col-span-2 sm:col-auto'>
      ...
      if (sm) {
          ...
          return (
            <>
              {selectsRange && showTimeSelect ? (
                <>
                  <Typography component='span' variant='subtitle2' fontWeight='thin' className='capitalize'>
                    Start {startDate?.getFullYear()}
                  </Typography>
                  <div className='flex justify-between mb-2'>
                    <div className='flex flex-col'>
                      <Typography component='div' variant='h4' fontWeight='light'>
                ...
                <>
                  ...
                  >
                    <div>
                      <Typography component='div' className='font-thin' variant='caption'>
                        {startDate?.getFullYear() ?? '-'}
                      </Typography>
                      {startDate ? `${monthNames[startDate.getMonth()]} ${startDate.getDate()}` : 'Start'}
                    </div>
                    <span>-</span>
                    <div>
                      <Typography component='div' className='font-thin' variant='caption'>
                      ...
                      {endDate ? `${monthNames[endDate.getMonth()]} ${endDate.getDate()}` : 'End'}
                    </div>
                  </Typography>
                </>
              ) : showTimeSelect ? (
                <>
                  ...
                  <div className='flex justify-between items-end'>
                    <div className='flex flex-col'>
                      ...
                      <Typography component='div' variant='h2' fontWeight='light'>
                        {startDate ? `${monthNames[startDate.getMonth()]} ${startDate.getDate()}` : '--'}
                      </Typography>
                    </div>
                ...
                <>
                  ...
                  <Typography component='div' variant='h2' fontWeight='light' gutterBottom>
                    {startDate
            ...
            <>
              {showTimeSelect && (
                <Tabs
                  ...
                  color='primary'
                  className='px-4'
                  onChange={(_, v) => setDateState(p => ({ ...p, activeIndex: Number(v) }))}
                >
                  <Tab
                    value='0'
                  ...
                  <Tab
                    value='1'
                    label={<IconClock color={!dateState.startHour ? 'secondary' : 'primary'} fontSize={20} />}
        ...
        return (
          <>
            ...
            <Carousel activeIndex={dateState.activeIndex} classNames={{ root: 'w-[302px]' }}>
              <div
                ...
                >
                  <div className='h-full flex p-1'>
                    <TimeSelector
                      ...
                      registerHourRef={(dateType, el) => (el ? hourInputRefs.current.set(dateType, el) : undefined)}
                      registerMinuteRef={(dateType, el) => (el ? minuteInputRefs.current.set(dateType, el) : undefined)}
                      ...
                      <div className='h-full flex p-1 border-t border-divider'>
                        <TimeSelector
                          ...
                          registerHourRef={(dateType, el) => {
                            return el ? hourInputRefs.current.set(dateType, el) : undefined
                          }}
                          registerMinuteRef={(dateType, el) => {
                            return el ? minuteInputRefs.current.set(dateType, el) : undefined
                          }}
                          onFocus={(focusedElement, activeInput) => {
                            activeInputRef.current = activeInput
                            setDateState(p => ({ ...p, focusedElement }))
      ...
      return (
        <div className='flex'>
          <div className='flex-1'>
            <div className={`relative overflow-hidden px-4 pt-4 ${isClearable ? '' : 'pb-4'}`}>
              <IconButton
                type='button'
                variant='text'
                sizes='small'
          ...
          {showTimeSelect && (
            <div
              style={{ maxHeight: sm ? (isClearable ? 625 : 590) : isClearable ? 335.5 : 310 }}
...
type DateChangeType = 'day' | 'month' | 'year' | 'date' | 'selector-month' | 'selector-year'
...
const InputDate = (props: InputDateProps) => {
  ...
  return (
    <>
      ...
      >
        <div ref={calendarRef} onKeyDown={handleKeyDown} className='w-full' tabIndex={0} aria-label='Kalender'>
