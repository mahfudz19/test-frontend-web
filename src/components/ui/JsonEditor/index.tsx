...
import { DebounceCallback } from "src/components/util/UI/DebounceUI";
...
const LineCount = ({ internalValue }: { internalValue: string }) => {
  ...
  useEffect(
    () => setLineCount(internalValue?.split?.("\n")?.length),
    [internalValue]
  );
...
const Search = (props: SearchProps) => {
  ...
  const scrollToMatch = useCallback(
    (
      matchIndex: number,
      matches: { index: number; pos: number }[],
      searchText: string
    ) => {
      if (
        matchIndex < 0 ||
      ...
      requestAnimationFrame(() => {
        ...
        const lineHeight = 20;
        const linesBefore =
          internalValue.substring(0, match.pos).split("\n").length - 1;
  ...
  return (
    <>
      <IconButton
        ...
        onClick={handleUndo}
        disabled={
          currentIndexRef.current <= 0 || historyRef.current.length <= 1
        }
      >
        <IconUndo />
      </IconButton>
...
function JsonEditor<T>({
    ...
    if (e.key === "Tab") {
      ...
      if (selectionStart !== selectionEnd) {
        ...
        if (e.shiftKey) {
          ...
          const newValue =
            beforeCursor + unindentedLines.join("\n") + afterCursor;
        ...
        if (e.shiftKey) {
          ...
          if (indentMatch && indentMatch[0].length > 0) {
            ...
            const newBeforeCursor = beforeCursor.substring(
              0,
              beforeCursor.length - unindentAmount
            );
      ...
      if (selectionStart === selectionEnd) {
        for (const [open, close] of Object.entries(autoClosePairs)) {
          if (
            ...
            e.preventDefault();
            const newValue =
              beforeCursor.slice(0, -1) + afterCursor.substring(1);
            setInternalValue(newValue);
            setCursorPosition(selectionStart - 1);
            saveToHistory(newValue, selectionStart - 1);
            return;
  ...
  const handleChange = (e: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    try {
      const value: T | undefined = newVal.trim()
        ? JSON.parse(newVal)
        : undefined;
      setParseError(null);
      onChange?.({
        ...e,
        target: { ...e.target, name: rest.name || e.target.name, value },
      });
    } catch {
  ...
  return (
    <div className="relative rounded-lg border border-divider">
      ...
      <div
        ref={scrollableRef}
        className="rounded-[inherit] flex-col overflow-auto max-h-96"
      >
        <div className="flex">
          ...
          <textarea
            ...
            spellCheck={false}
            className={twMerge(
              "font-mono text-sm flex-1 p-2 min-h-[300px] outline-none resize-none",
              className
            )}
            {...rest}
          />
      ...
      {parseError && (
        <div className="bg-error-light/30 border-t border-divider text-error-dark p-2 text-sm">
          {parseError}
        </div>
      )}
    </div>
