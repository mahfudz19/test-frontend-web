...
const useTimeState = (
  ...
  const safeSetTime = useCallback((updater: (prev: TimeState) => TimeState) => {
    setTime((prev) => {
      ...
      if (prev.h === next.h && prev.m === next.m && prev.p === next.p)
        return prev;
      return next;
    });
  }, []);
    ...
    <button
      ...
      className={twMerge(
        ...
        active
          ...
          : "hover:bg-primary-main/25 text-text-primary"
      )}
...
const AnalogClock = memo((props: AnalogClockProps) => {
  ...
  return (
    <div className="relative h-[200px] w-[200px] mx-auto rounded-full">
      ...
      {[...innerHours, ...(is24HourFormat ? outerHours : [])].map(
        ({ hour, x, y }) => (
          <ClockButton
            key={`hour-${hour}`}
            x={x}
...
const InputTime = forwardRef<
  HTMLInputElement,
  TextFieldProps & { value?: string; defaultValue?: string }
>((propd, ref) => {
  const {
    label,
    error,
