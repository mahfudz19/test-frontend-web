...
export type MenuItemProps = HtmlHTMLAttributes<HTMLLIElement> &
...
const variants = cva(
  ...
  ],
  {
    variants: {
      variant: {
        outlined: [
          "border-[0.0625rem] flex items-center justify-between gap-1 text-sm p-2 my-1 rounded-lg",
        ],
        contained: ["p-2"],
      },
    },
...
const IconStart = ({ iconStart, classNames, selected }: IconStartProps) => {
  ...
  if (
    selected &&
    typeof classNames?.icon_start !== "string" &&
    classNames?.icon_start?.selected
  ) {
...
const IconEnd = ({ IconEnd, classNames, selected }: IconEndProps) => {
  ...
  if (
    selected &&
    typeof classNames?.icon_end !== "string" &&
    classNames?.icon_end?.selected
  ) {
...
const Content = ({ children, classNames, selected }: ContentProps) => {
  ...
  if (
    selected &&
    typeof classNames?.content !== "string" &&
    classNames?.content?.selected
  ) {
    iconSelected = classNames?.content?.selected;
  }

  return (
    <span className={twMerge("flex-grow", iconClass, iconSelected)}>
      {children}
    </span>
  );
};
