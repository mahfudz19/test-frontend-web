...
const Category = ({category, setCategory}:{category:string; setCategory:(v:string)=>void}) => {
  type ICategory = {
    "data": { "id": string; "userId": string; "name": string; "createdAt": string; "updatedAt": string }[]
    "totalData": number
    "currentPage": number
    "totalPages": number
}
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const res = await axios.get<ICategory>(`https://test-fe.mysellerpintar.com/api/categories?limit=999`);
...
const UserArticlesPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`https://test-fe.mysellerpintar.com/api/articles?page=${page}&category=${category}`);
        ...
        <ul>
          {articles.map((article) => (
            <li key={article.id} className="mb-6 border p-4 rounded shadow">
              ...
              {article.imageUrl && (
                <Image width={100} height={200} src={article.imageUrl} alt={article.title} className="my-2 rounded" />
              )}
              <div
